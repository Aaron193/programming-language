...
AstBytecodeEmitter::AstBytecodeEmitter(Compiler& compiler,
                                       const AstFrontendResult& frontend)
    : m_compiler(compiler), m_frontend(frontend) {}

bool AstBytecodeEmitter::emitModule() {
...
std::string AstBytecodeEmitter::tokenText(const Token& token) const {
    return std::string(token.start(), token.length());
}

TypeRef AstBytecodeEmitter::resolveTypeExpr(const AstTypeExpr* typeExpr) const {
    if (typeExpr == nullptr) {
...
size_t AstBytecodeEmitter::lastModuleLine() const {
    return safeLine(m_frontend.terminalLine);
}

void AstBytecodeEmitter::errorAtSpan(const SourceSpan& span,
                                     const std::string& message) {
    m_compiler.errorAtSpan(span, message);
}

void AstBytecodeEmitter::errorAtNode(const AstNodeInfo& node,
                                     const std::string& message) {
    errorAtSpan(node.span, message);
}

void AstBytecodeEmitter::errorAtToken(const Token& token,
                                      const std::string& message) {
    errorAtSpan(token.span(), message);
}

void AstBytecodeEmitter::errorAtLine(size_t line, const std::string& message) {
    m_compiler.errorAtLine(safeLine(line), message);
}

TypeRef AstBytecodeEmitter::nodeType(AstNodeId id) const {
...
TypeRef AstBytecodeEmitter::nodeType(const AstNodeInfo& node) const {
    return nodeType(node.id);
}

bool AstBytecodeEmitter::nodeConst(AstNodeId id) const {
    auto it = m_frontend.semanticModel.nodeConstness.find(id);
    return it != m_frontend.semanticModel.nodeConstness.end() && it->second;
}

void AstBytecodeEmitter::emitByte(uint8_t byte, size_t line) {
    m_compiler.emitByte(byte, safeLine(line));
}

void AstBytecodeEmitter::emitBytes(uint8_t byte1, uint8_t byte2, size_t line) {
    m_compiler.emitBytes(byte1, byte2, safeLine(line));
}

void AstBytecodeEmitter::emitConstant(Value value, size_t line) {
    emitBytes(OpCode::CONSTANT, m_compiler.makeConstant(value), line);
}

int AstBytecodeEmitter::emitJump(uint8_t instruction, size_t line) {
...
void AstBytecodeEmitter::patchJump(int offset) {
...
void AstBytecodeEmitter::emitLoop(int loopStart, size_t line) {
...
void AstBytecodeEmitter::emitReturn(size_t line) {
    emitByte(OpCode::NIL, line);
    emitByte(OpCode::RETURN, line);
}

void AstBytecodeEmitter::emitCoerceToType(const TypeRef& sourceType,
...
void AstBytecodeEmitter::emitCheckInstanceType(const TypeRef& targetType,
                                               size_t line) {
...
bool AstBytecodeEmitter::emitCompoundBinary(TokenType assignmentType,
...
void AstBytecodeEmitter::beginScope() {
    m_compiler.currentContext().scopeDepth++;
}

void AstBytecodeEmitter::endScope(size_t line) {
    m_compiler.currentContext().scopeDepth--;
...
void AstBytecodeEmitter::defineVariable(uint8_t global, size_t line) {
...
void AstBytecodeEmitter::emitVariableRead(const Token& name, size_t line) {
...
TypeRef AstBytecodeEmitter::lookupClassFieldType(
...
TypeRef AstBytecodeEmitter::lookupClassMethodType(
...
const AstImportedModuleInterface* AstBytecodeEmitter::importedModuleForNode(
...
void AstBytecodeEmitter::emitExportName(const Token& nameToken, size_t line) {
...
void AstBytecodeEmitter::emitArguments(const std::vector<AstExprPtr>& arguments,
...
Compiler::CompiledFunction AstBytecodeEmitter::compileFunction(
...
void AstBytecodeEmitter::emitFunctionDecl(const AstFunctionDecl& functionDecl) {
    ...
    if ((!functionType || functionType->isAny()) &&
        ...
        functionType =
            m_frontend.functionSignatures.at(tokenText(functionDecl.name));
    }
    uint8_t variable = 0;
...
void AstBytecodeEmitter::emitClassDecl(const AstClassDecl& classDecl) {
...
void AstBytecodeEmitter::emitVarDecl(const AstVarDeclStmt& stmt,
...
void AstBytecodeEmitter::emitDestructuredImport(
    const AstDestructuredImportStmt& stmt, size_t line) {
    if (!stmt.initializer ||
    ...
    const auto& importExpr = std::get<AstImportExpr>(stmt.initializer->value);
...
void AstBytecodeEmitter::emitItem(const AstItem& item) {
    std::visit(
...
void AstBytecodeEmitter::emitStmt(const AstStmt& stmt) {
    std::visit(
...
void AstBytecodeEmitter::emitAssignmentToVariable(
    const AstIdentifierExpr& target, const Token& op, const AstExpr* valueExpr,
    size_t line) {
...
void AstBytecodeEmitter::emitAssignmentToMember(const AstMemberExpr& target,
                                                const Token& op,
                                                const AstExpr* valueExpr,
                                                size_t line) {
...
void AstBytecodeEmitter::emitAssignmentToIndex(const AstIndexExpr& target,
                                               const Token& op,
                                               const AstExpr* valueExpr,
                                               size_t line) {
    emitExpr(*target.object);
...
void AstBytecodeEmitter::emitExpr(const AstExpr& expr) {
    std::visit(
